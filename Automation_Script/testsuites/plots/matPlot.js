casper.test.begin("Test script for matPlot", 8, function suite(test) {
    ...
    var URL;
    var NB_ID = 'a254c2194edeac3ce48c'
    ...
    casper.then(function(){
        this.wait(3000);
        this.waitForSelector('.live-plot-container', function () {
            this.test.assertExists('.live-plot-container', 'Confirmed that cell has prodeced expected plot using "matPlot" library');
        });
    });
    ...
    casper.then(function (){
        ...
        logo = this.evaluate(function() {
            var imgUrl = document.querySelector('.live-plot-scroller').getAttribute('src');
            return __utils__.getBase64(imgUrl);
        });
        if (logo === true ){
            this.echo("Plot has been generated");
        }
        else
        {
            this.echo("Plot has not been generated")
        }
    });
