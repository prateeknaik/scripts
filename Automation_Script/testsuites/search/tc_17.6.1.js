...
casper.test.begin(" Numbers and Special Characters Ex: (111) )", 5, function suite(test) {
    ...
    var item = '(2015/28/08)';//item to be searched
    ...
    casper.then(function(){
		if (this.visible('#input-text-search')) {
            ...
            if (counter >0)
            {
				this.test.pass("search feature is working for both numbers and special characters");
			}
			else
				{
					this.test.fail("search feature is not working for both numbers and special characters");
				}
		});
