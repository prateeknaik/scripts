...
//Begin Tests
casper.test.begin("  Both Lowercase and Uppercase letters -Ex: rnORm", 6, function suite(test) {
    ...
    casper.then(function(){
		if (this.visible('#input-text-search')) {
            ...
            if (counter >0)
            {
				this.test.pass("search feature is working for both upper case and lower case for lower case keywords");
			}
			else
				{
					this.test.fail("search feature is not working for both upper case and lowercase keywords");
				}
		});
