...
casper.test.begin(" Alphabets and Numbers Ex: abc123", 5, function suite(test) {
    ...
    casper.then(function(){
		if (this.visible('#input-text-search')) {
            ...
            if (counter >0)
            {
				this.test.pass("search feature is working for both both alphabets and numbers");
			}
			else
				{
					this.test.fail("search feature is not working for both alphabets and numbers");
				}
		});
